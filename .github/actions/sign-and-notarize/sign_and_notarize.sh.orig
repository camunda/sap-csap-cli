...
#
# 1) Moves out top-level items in <BUILD_DIR> whose names begin with:
#      - camunda-zeebe
...
#
# Usage:
#   ./exclude_sign_notarize_reinject_absolute.sh <BUILD_DIR> \
...
if [ "$#" -ne 5 ]; then
  echo "Usage: $0 <BUILD_DIR> \"Developer ID Application: Cert (TEAMID)\" <AppleIDEmail> <AppPassword> <TeamID>"
  exit 1
fi
...
find "$BUILD_DIR" -maxdepth 1 \( -type f -o -type d \) -print0 | while IFS= read -r -d '' item; do
  base="$(basename "$item")"
  # Skip if it's c8run itself or hidden
  if [ "$base" = "$(basename "$BUILD_DIR")" ] || [[ "$base" == .* ]]; then
    continue
  fi
...
sign_macho_in_folder "$BUILD_DIR"
sign_jars_in_folder "$BUILD_DIR"
...
(
  ...
  /usr/bin/ditto -c -k --keepParent "$(basename "$BUILD_DIR")" csap_signed.zip
)
SIGNED_ZIP_PATH="$(cd "$(dirname "$BUILD_DIR")" && pwd -P)/csap_signed.zip"
...
# )
# FINAL_ZIP="$(dirname "$BUILD_DIR")/c8run_complete.zip"
...
cat <<EOM
Note: csap_signed.zip includes the previously excluded items, which remain
unsigned/unnotarized if they contain Mach-O. Apple only notarized csap_signed.zip.
EOM
